...
export class PrismaAccessRequestRepository implements AccessRequestRepository {
  ...
  async list(filters: RequestFilters) {
    ...
    if (filters.query) where.OR = ['requesterName', 'requesterEmail', 'systemName'].map((field) => ({ [field]: { contains: filters.query, mode: 'insensitive' } }));
  ...
  async review(id: string, input: ReviewRequestInput) {
    return this.prisma.$transaction(async (transaction) => {
      const result = await transaction.accessRequest.updateMany({ where: { id, version: input.version, status: 'PENDING' }, data: { status: input.decision, reviewerNote: input.note, version: { increment: 1 } } });
      if (result.count === 0) {
