...
export class MemoryAccessRequestRepository implements AccessRequestRepository {
  ...
  async list(filters: RequestFilters) {
    ...
    const filtered = this.rows.filter((row) => (!filters.status || row.status === filters.status) && (!query || [row.requesterName, row.requesterEmail, row.systemName].some((value) => value.toLowerCase().includes(query))));
    const offset = (filters.page - 1) * filters.pageSize;
    return { total: filtered.length, items: filtered.slice(offset, offset + filters.pageSize).map((row) => ({ ...row })) };
  }
  async findById(id: string) { const row = this.rows.find((candidate) => candidate.id === id); return row ? { ...row } : null; }
  async summary() { return { pending: this.rows.filter((row) => row.status === 'PENDING').length, approved: this.rows.filter((row) => row.status === 'APPROVED').length, rejected: this.rows.filter((row) => row.status === 'REJECTED').length }; }
  async review(id: string, input: ReviewRequestInput) {
    ...
    row.status = input.decision; row.reviewerNote = input.note; row.version += 1; row.updatedAt = new Date().toISOString();
    return { ...row };
  }
}
